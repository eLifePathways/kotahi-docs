---
title: "Publish to your public website"
description: "Day-to-day editing of your public website — pages, layout, header and footer."
sidebar:
  order: 11
reviewStatus: converted-unverified
sourceNote: "Converted from docs.kotahi.community/advanced-kotahi/cmsuse.html."
---

*Editing the website your readers see.*

Kotahi can build and host your **public website** — the site where your published
content appears. Content can go to that site, to external endpoints, or to both.

:::note[Kotahi calls this the CMS]
In Kotahi's menus and settings this is labelled **CMS**, and the current version of
it is **BATS CMS**. This page uses the on-screen labels so you can follow along, but
"CMS" and "your public website" mean the same thing here.
:::

The controls are built for creating straightforward publishing sites quickly, and
you will find them under Settings→CMS. If you choose the first of the items (‘Pages’) below the CMS selector, you will see the following.

![CMS 'Pages' screen with a page list (About us, Contact us, FAQ, Submit) and an add button, a 'Page title' field, a URL stub field, a rich-text editor holding body text above an inserted logo figure, and 'Publish' and 'Delete' buttons with edited and published dates.](../../../assets/screenshots/f93eed030982-1500w.png)

Create new pages here and edit them with the editor provided.

To add a new page, click the ‘+’ icon. The page won’t be added to the publishing site until you include it in the menu and choose to publish it. You can publish pages by clicking on the **Publish** action.

You can fill out the new page title, the ‘stub’ (effectively the URL the page will appear on), and then add the initial content. The editor also supports image insertion.

## Layout

Layout of the page is accessed by clicking ‘Layout’ in the CMS menu.

![CMS 'Layout' screen with a 'Brand logo' drag-and-drop upload area, 'Brand Color' swatches for primary and secondary colours, a 'Header' list of pages with tick boxes and drag handles, and a 'Footer' section for partner logo uploads with website URL fields.](../../../assets/screenshots/e38ce350aa62-1500w.png)

Here you have some basic settings for the brand, uploading a logo, and selecting a primary and secondary color.

## Header

In the Header section, you can control the menu. The header section appears on every page. Pages ordered top to bottom are displayed from left to right as links in the header. Only selected pages are displayed when published.

!['Header' settings panel with the instruction 'Use checkbox to show and hide the page in the menu. Click and Drag to order them.', listing the site's pages with tick boxes; only three are ticked, and each row has a drag handle.](../../../assets/screenshots/bc1f00070997-2500w.png)

Pages selected to appear in the header section on the publishing site (Flax).

![Published site header bar: the green Kotahi logo at the left and, at the right, the three navigation links selected in the CMS header settings — 'About us', 'FAQ' and 'Submit'.](../../../assets/screenshots/fad7b38d230e-2500w.png)

View of published header page links on the publishing site (Flax).

## Footer

In the following options, you can configure what appears in the footer, starting with partner logos and associated website link. Footers are displayed on all pages.

![CMS 'Footer' settings 'Partners' area with a 'Drag and drop your files here' upload zone above three uploaded logo files, each with a 'Remove' link and a website URL field.](../../../assets/screenshots/d3af5580fca4-2500w.png)

Next you can set the footer text.

!['Footer Text' rich-text editor with a formatting toolbar (paragraph style, bold, italic, link, lists, table options, image, full screen) above a postal address and a 'Keep up to date' newsletter registration line.](../../../assets/screenshots/33dc762aa321-1000w.png)

Lastly you can include links to additional pages in the footer (possibly items not in the topmost menu).

!['Footer Page links' setting listing six draggable rows with tick boxes, of which only 'Contact us' is ticked; each row has a drag handle for reordering.](../../../assets/screenshots/1dbad6c5b9f1-750w.png)

When you press ‘Publish’, the updated settings will be pushed to your publishing site.

## Status

This option allows you to hide your publishing endpoint from the web. Your publishing website will only be visible to those who have access to the ‘Draft’ link.

:::note[Screenshot being refreshed]
This screen is being re-captured against the current Kotahi release. Until then, here is what it shows.

**The screen shows:** CMS Layout page with the 'Status' panel highlighted: a ticked 'Draft' checkbox, the note 'Your publishing website will only be visible to those who have access to the Draft link', and the draft URL beneath.
:::

